...
function initDOM() {
  const ids = [
    ...
    "enableAlertTool",
    "enableCssStyleTool",
    "enableCogneeRemember",
    "enableCogneeBatchRemember",
    "enableCogneeCognify",
    "enableCogneeRecall",
    "enableCogneeForget",
    "enableNotionSearch",
...
async function connect() {
  try {
    ...
    if (!isGroundingEnabled) {
      ...
      // Add Cognee memory tools if enabled
      if (elements.enableCogneeRemember && elements.enableCogneeRemember.checked) {
        const cogneeRemember = new CogneeRememberTool();
        state.client.addFunction(cogneeRemember);
        console.log("✅ Cognee remember tool enabled");
      }
      if (elements.enableCogneeBatchRemember && elements.enableCogneeBatchRemember.checked) {
        const cogneeBatchRemember = new CogneeBatchRememberTool();
        state.client.addFunction(cogneeBatchRemember);
        console.log("✅ Cognee batch remember tool enabled");
      }
      if (elements.enableCogneeCognify && elements.enableCogneeCognify.checked) {
        const cogneeCognify = new CogneeCognifyTool();
        state.client.addFunction(cogneeCognify);
        console.log("✅ Cognee cognify tool enabled");
      }
      if (elements.enableCogneeRecall && elements.enableCogneeRecall.checked) {
        const cogneeRecall = new CogneeRecallTool();
        state.client.addFunction(cogneeRecall);
        console.log("✅ Cognee recall tool enabled");
      }
      if (elements.enableCogneeForget && elements.enableCogneeForget.checked) {
...
// Handle tool calls (supports async functions)
async function handleToolCalls(functionCalls) {
  ...
  for (let index = 0; index < functionCalls.length; index++) {
    ...
    let result;
    try {
      const func = state.client.functionsMap[functionName];
      if (!func) {
        throw new Error(`Unknown function: ${functionName}`);
      }

      // Call the function (may be async)
      result = await func.runFunction(parameters);
